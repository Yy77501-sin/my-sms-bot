...
def get_or_create_user(user_id, username="", first_name=""):
    try:
        ...
        print(f"DB Error: {e}")
        return 0.0, 0.0, 0, 0, first_name, username
...
class SimpleHTTPRequestHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        self.end_headers()
        self.wfile.write(b"Number SMS Store is Running Protected 24/7!")
...
# حالة المستخدمين والأدمن لإدخال النصوص
...
# ----------------- معالجة النصوص وحالات البحث الآمنة -----------------
...
def handle_all_text_messages(message):
    try:
        ...
        if not state:
            return  # رسالة عادية غير مرتبطة بطلب إدخال

        action = state.get("action")
        
        # 1. البحث الآمن عن دولة لتطبيق معين
        if action == "search_country_for_app":
            ...
            if not matched_countries:
                msg_text = f"🔍 **لم يتم العثور على دولة مطابقة لـ:** `{text}`\n\n👉 يمكنك تجربة اسم آخر (مثل: مصر، روسيا، إندونيسيا، أو الرمز +20)."
        ...
        # 2. بحث الأدمن عن مستخدم بالـ ID أو اليوزرنيم
...
# ----------------- تشغيل البوت -----------------
def start_bot():
    print("⏳ جاري تهيئة البوت وقاعدة البيانات...")
    time.sleep(2)
    try:
    ...
    print("🚀 البوت بدأ العمل بنظام البحث الآمن ومحمي من الانطفاء 24/7...")
    while True:
        try:

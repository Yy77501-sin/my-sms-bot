...
# 1. قاعدة بيانات محلية لحفظ رصيد ومشتريات كل مستخدم
...
def get_or_create_user(user_id, username="", first_name=""):
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    cursor.execute('SELECT balance, rub_balance, orders_count FROM users WHERE user_id = ?', (str(user_id),))
    row = cursor.fetchone()
    if row:
        conn.close()
        ...
        cursor.execute(
            'INSERT INTO users (user_id, username, first_name, balance, rub_balance, orders_count) VALUES (?, ?, ?, ?, ?, 0)',
            (str(user_id), username, first_name, init_bal, init_rub)
        )
        conn.commit()
        conn.close()
        return init_bal, init_rub, 0
...
class SimpleHTTPRequestHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        self.end_headers()
        self.wfile.write(b"Number SMS Secure Store is Running Live 24/7!")
...
def buy_hero_number(service_code, country_id):
    ...
    if resp.startswith("ACCESS_NUMBER:"):
        ...
        return False, "رصيد السيرفر غير كافٍ، تواصل مع الإدارة."
...
# مؤقت فحص الكود الذكي (مع الإلغاء التلقائي الإجباري عند الدقيقة 18 لمنع أي خصم مالي)
def monitor_sms_code(chat_id, message_id, order_id, phone_number, app_name, country_title, cost_str, time_now_str, time_exp_str, cost_usd, cost_rub, user_id):
    ...
    # 18 دقيقة كحد أقصى (1080 ثانية) قبل وصول حد الـ 20 دقيقة الخاص بالمزود
    max_duration = 1080 
    start_time = time.time()
    ...
    # إذا مرت 18 دقيقة ولم يصل الكود -> إلغاء إجباري من المزود لحماية الحساب من الخصم
    if order_id in active_orders:
        set_status(order_id, 8)  # إلغاء الطلب من الموقع قبل الدقيقة 20
        update_user_balance(user_id, cost_usd, cost_rub)
        active_orders.pop(order_id, None)
        bot.send_message(
            chat_id,
            f"⌛ **انتهت مهلة الانتظار للرقم `+{phone_number}` ولم يصل الكود.**\n\n"
            f"🛡️ **حماية للمستخدم والحساب:** تم إلغاء الطلب من المزود واسترجاع كامل المبلغ ({cost_str}) إلى محفظتك تلقائياً."
        )
...
def start_command(message):
    try:
        ...
        bal_usd, bal_rub, orders = get_or_create_user(user_id, username, first_name)
...
def router_callback(call):
    try:
        ...
        data = call.data
        bal_usd, bal_rub, orders = get_or_create_user(user_id, call.from_user.username, call.from_user.first_name)
        ...
        if data == "back_to_main":
            ...
            markup = types.InlineKeyboardMarkup(row_width=2)
            markup.add(
                types.InlineKeyboardButton("➕ شحن رصيد لمستخدم", callback_data="admin_add_bal"),
            ...
            bot.edit_message_text(
                chat_id=chat_id,
                message_id=call.message.message_id,
                text="👑 **لوحة تحكم إدارة المتجر:**\n\nيمكنك فحص رصيد السيرفر أو شحن الرصيد:",
                reply_markup=markup,
                parse_mode="Markdown"
            )
            ...
            bot.send_message(chat_id, f"💳 **رصيد حسابك في مزود Hero SMS هو:** `{bal} $`\n✅ السيرفر متصل ومحمي.", parse_mode="Markdown")

    except Exception as e:
        print(f"Callback error: {e}")

# أمر الأدمن لشحن الرصيد: /add 123456 5
...
def admin_add_balance_cmd(message):
    ...
    try:
        ...
        bot.reply_to(message, f"✅ **تم شحن {amount} $ بنجاح للمستخدم `{target_uid}`.**")
        try:
            bot.send_message(target_uid, f"🎉 **تهانينا!**\nتم شحن رصيد محفظتك بمبلغ: **{amount:.2f} $** بنجاح.")
        except Exception:
            pass
    except Exception:
        bot.reply_to(message, "⚠️ **طريقة الاستخدام:**\n`/add <User_ID> <Amount_USD>`")
...
def start_bot():
    ...
    print("🚀 البوت بدأ العمل ومحمي بالكامل 24/7...")
    while True:
        try:

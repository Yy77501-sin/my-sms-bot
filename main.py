...
# دالة فحص رصيد Hero SMS باختصار ودقة
def fetch_hero_balance():
    ...
    for ep in endpoints:
        try:
            res = requests.get(f"{ep}?api_key={API_KEY}&action=getBalance", timeout=8)
            text = res.text.strip()
            if text.startswith("ACCESS_BALANCE:"):
                return True, text.split(":")[1]
            elif text == "BAD_KEY":
                return False, "المفتاح (API Key) غير صالح"
        except Exception:
            continue
    return False, "تعذر الاتصال بالمزود حالياً"
...
def main_callback_handler(call):
    ...
    # فحص رصيد المزود من داخل لوحة الأدمن (المصحح والآمن)
        ...
        if success:
            ...
            bot.send_message(chat_id, f"⚠️ نتيجة الفحص: {bal}")

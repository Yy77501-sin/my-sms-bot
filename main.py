...
# 1. قاعدة بيانات متطورة للمستخدمين والحظر
...
def get_or_create_user(user_id, username="", first_name=""):
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    cursor.execute('SELECT balance, rub_balance, orders_count, is_banned, first_name, username FROM users WHERE user_id = ?', (str(user_id),))
    row = cursor.fetchone()
    now_str = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
    if row:
        # تحديث الاسم واليوزرنيم إذا تغيرا
        if username or first_name:
            cursor.execute('UPDATE users SET username = ?, first_name = ? WHERE user_id = ?', (username, first_name, str(user_id)))
            conn.commit()
        conn.close()
        return row[0], row[1], row[2], row[3], row[4], row[5]
    else:
        init_bal = 100.0 if str(user_id) == str(ADMIN_ID) else 0.0
        init_rub = 7000.0 if str(user_id) == str(ADMIN_ID) else 0.0
        cursor.execute(
            'INSERT INTO users (user_id, username, first_name, balance, rub_balance, orders_count, is_banned, joined_at) VALUES (?, ?, ?, ?, ?, 0, 0, ?)',
            (str(user_id), username, first_name, init_bal, init_rub, now_str)
        )
        conn.commit()
        conn.close()
        return init_bal, init_rub, 0, 0, first_name, username
...
def get_single_user_info(user_id):
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    cursor.execute('SELECT user_id, username, first_name, balance, rub_balance, orders_count, is_banned, joined_at FROM users WHERE user_id = ?', (str(user_id),))
    row = cursor.fetchone()
    conn.close()
    return row
...
class SimpleHTTPRequestHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        self.end_headers()
        self.wfile.write(b"Number SMS Store with Full Admin Control is Live 24/7!")
...
# حالة الأدمن الحالية (لإدخال المبالغ أو الـ ID أو الإذاعة)
admin_states = {}  # {admin_id: {"action": "action_name", "target_uid": "..."}}
...
def fetch_hero_balance():
    ...
    return False, "تعذر الاتصال بالمزود"
...
def build_servers_keyboard(app_code):
    ...
    markup.add(types.InlineKeyboardButton("• 🚀 البحث عن دولة 🧩 •", callback_data=f"list_countries_{app_code}_all"))
    markup.add(
...
# ----------------- معالجة النصوص وحالات الإدارة -----------------

@bot.message_handler(func=lambda msg: str(msg.from_user.id) == str(ADMIN_ID) and str(msg.from_user.id) in admin_states)
def handle_admin_text_input(message):
    ...
    # 1. البحث عن مستخدم بالـ ID
    if action == "search_user":
        user_info = get_single_user_info(text)
        if not user_info:
            bot.reply_to(message, f"❌ لم يتم العثور على مستخدم بالمعرف: `{text}`", parse_mode="Markdown")
            return
        
        uid, uname, fname, bal_usd, bal_rub, orders, is_banned, joined = user_info
        status_text = "🚫 محظور" if is_banned else "✅ نشط"
        card = (
            f"👤 **ملف المستخدم:**\n"
            f"━━━━━━━━━━━━━━━━━━\n"
            f"• الاسم: **{fname}**\n"
    ...
    # 2. إدخال مبلغ الشحن
        ...
        try:
            ...
            bot.reply_to(message, "❌ القيمة المدخلة غير صحيحة، يرجى كتابة أرقام فقط (مثال: 5.0).")
    ...
    # 5. الإذاعة العامة
        ...
        for u in users:
            ...
            try:
                ...
                continue
        bot.send_message(admin_id, f"✅ **اكتملت الإذاعة بنجاح!**\nتم التوصيل إلى **{sent_count}** مستخدم.")
...
def router_callback(call):
    try:
        ...
        # عرض قائمة المستخدمين والإحصائيات
            ...
            # عرض آخر 15 مستخدم كأزرار قابلة للضغط المباشر
            for u in users[:15]:
        ...
        # طلب البحث عن ID
            ...
            if str(user_id) != str(ADMIN_ID):
                return
            bot.answer_callback_query(call.id)
            admin_states[str(user_id)] = {"action": "search_user"}
            bot.send_message(chat_id, "🔍 **أرسل الآن معرف المستخدم (User ID) للبحث عنه:**", parse_mode="Markdown")
        ...
        # بدء شحن رصيد
            ...
            bot.answer_callback_query(call.id)
            admin_states[str(user_id)] = {"action": "add_balance_amount", "target_uid": target_uid}
        ...
        # بدء خصم رصيد
            ...
            bot.answer_callback_query(call.id)
            admin_states[str(user_id)] = {"action": "sub_balance_amount", "target_uid": target_uid}
        ...
        # مراسلة خاصة
            ...
            bot.answer_callback_query(call.id)
            admin_states[str(user_id)] = {"action": "send_direct_message", "target_uid": target_uid}
        ...
        # بدء إذاعة عامة
            ...
            if str(user_id) != str(ADMIN_ID):
                return
            bot.answer_callback_query(call.id)
            admin_states[str(user_id)] = {"action": "broadcast_all"}
...
# أوامر سريعة للأدمن في المحادثة
...
def admin_add_cmd(message):
    ...
    try:
        ...
        update_user_balance(target_uid, amount, amount_rub)
        bot.reply_to(message, f"✅ **تم شحن {amount} $ بنجاح للمستخدم `{target_uid}`.**", parse_mode="Markdown")
        try:
            bot.send_message(target_uid, f"🎉 **تم شحن رصيد محفظتك بمبلغ: {amount:.2f} $ بنجاح.**")
        except Exception:
            pass
    except Exception:
        bot.reply_to(message, "⚠️ استخدام الأمر: `/add <User_ID> <المبلغ>`")

@bot.message_handler(commands=['sub'])
def admin_sub_cmd(message):
    if str(message.from_user.id) != str(ADMIN_ID):
        return
    try:
        parts = message.text.split()
        ...
        amount_rub = amount * 90.0
        update_user_balance(target_uid, -amount, -amount_rub)
        bot.reply_to(message, f"✅ **تم خصم {amount} $ بنجاح من المستخدم `{target_uid}`.**", parse_mode="Markdown")
    except Exception:
        bot.reply_to(message, "⚠️ استخدام الأمر: `/sub <User_ID> <المبلغ>`")

# ----------------- تشغيل البوت -----------------
def start_bot():
    print("⏳ جاري تهيئة البوت ولوحة تحكم الإدارة الكاملة...")
    time.sleep(2)
    try:
    ...
    print("🚀 البوت بدأ العمل بنظام إدارة المستخدمين والتحكم الكامل 24/7...")
    while True:
        try:

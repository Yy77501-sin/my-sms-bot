...
# 1. خادم ويب متكامل لإرضاء فحص Render بنجاح
...
def callback_handler(call):
    ...
    # 3. قائمة الدول الأكثر وفرة
        ...
        bot.edit_message_text(
            chat_id=chat_id,
            message_id=call.message.message_id,
            text=f"🌍 **اختر الدولة لخدمة ({service_code}):**\n_💡 إذا نفدت الأرقام في دولة، جرب دولة أخرى من القائمة مباشرة:_",
            reply_markup=markup,
            parse_mode="Markdown"
        )
    ...
    # 4. تنفيذ الشراء
        ...
        if success:
            ...
            bot.edit_message_text(
                chat_id=chat_id,
                message_id=msg.message_id,
                text=f"❌ **تعذر حجز الرقم في ({country_code}):**\nالأرقام غير متوفرة حالياً في هذه الدولة.\n👉 **يرجى اختيار دولة أخرى (مثل إندونيسيا، روسيا، كازاخستان، كينيا)**.",
                parse_mode="Markdown"
            )
        ...
        bot.send_message(
            ...
            "2. اختر الدولة (مثل: إندونيسيا، كازاخستان، روسيا).\n"
            "3. انسخ الرقم وأدخله في التطبيق.\n"
            "4. انتظر كود التحقق في المحادثة."
        )

# ----------------- التشغيل الآمن مع حلقة المعالجة -----------------
if __name__ == "__main__":
    print("⏳ جاري تهيئة الاتصال وحذف أي اتصالات معلقة...")
    time.sleep(3)
    try:

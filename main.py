...
print(f"🚀 Initializing High-Performance Multi-Threaded Engine...")

# ----------------- شبكة اتصالات فائقة السرعة مع إعادة المحاولة -----------------
...
def get_db_connection():
    ...
    conn.execute("PRAGMA journal_mode=WAL;")  # وضع الكتابة فائق السرعة
...
class HealthCheckHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        self.end_headers()
        self.wfile.write(b"Number SMS Store Telegram Bot Ultra Engine Live 24/7!")
...
user_states = {}
user_click_lock = {}  # مانع التكرار والضغط العشوائي (Debounce)
...
def send_activation_log(user_id, country_title, service_name, phone, code, price_str, order_id):
    try:
        ...
        print(f"⚠️ Activation log notice: {e}")
...
# ----------------- معالجة الضغط على الأزرار (استجابة فورية فائقة السرعة) -----------------
...
def router_callback(call):
    try:
        ...
        # ⚡ استجابة فورية للتيليجرام لمنع علامة التحميل اللانهائية
        try:
            bot.answer_callback_query(call.id)
        except Exception:
            pass

        # مانع تكرار النقر السريع (Debounce Lock)
        now_ts = time.time()
        ...
        if data == "back_to_main":
            ...
            bot.send_message(chat_id, "🔍 **أرسل الآن اسم الدولة أو رمز مفتاحها (مثال: روسيا، كازاخستان، 966، أو 20):**")
        ...
        # باقي أقسام الشراء
            ...
            app_info = catalog.SERVICES.get(app_code, catalog.SERVICES["wa"])
            c_info = catalog.COUNTRIES.get(c_id, catalog.COUNTRIES.get("0", {}))
...
# ----------------- تشغيل المحرك فائق السرعة والمناعة -----------------
def start_bot():
    try:
    ...
    print("🚀 ULTRA MULTI-THREADED ENGINE RUNNING FOR THOUSANDS OF USERS 24/7...")

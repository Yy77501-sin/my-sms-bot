...
print(f"🚀 Initializing High-Performance Ultra Bot with Vouchers & Auto-Payment...")
...
# ----------------- قاعدة بيانات فائقة التحمل (WAL Mode + Vouchers) -----------------
...
def init_db():
    try:
        ...
        ''')
        # جدول قسائم وكروت الشحن (Vouchers / Gift Cards)
        cursor.execute('''
        ...
        ''')
        # جدول طلبات الدفع الآلي لمنع تكرار استخدام نفس العملية
        cursor.execute('''
        ...
        conn.commit()
        conn.close()
        print("✅ High-Speed WAL Database with Vouchers & Auto-Pay ready.")
    except Exception as e:
...
# دالة توليد كرت الشحن
def create_voucher(amount_usd, created_by):
    try:
        code = f"GIFT-{int(amount_usd)}USD-" + secrets.token_hex(3).upper()
        now_str = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute(
            'INSERT INTO vouchers (code, amount_usd, is_used, created_by, created_at) VALUES (?, ?, 0, ?, ?)',
            (code, float(amount_usd), str(created_by), now_str)
        )
        conn.commit()
        conn.close()
        return code
...
# دالة استرداد كرت الشحن
def redeem_voucher(code, user_id):
    try:
        clean_code = code.strip()
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute('SELECT amount_usd, is_used FROM vouchers WHERE code = ?', (clean_code,))
        row = cursor.fetchone()
        if not row:
            conn.close()
            return False, "❌ كود الشحن غير صحيح أو غير موجود."
        if row[1] == 1:
            conn.close()
            return False, "⚠️ تم استخدام كود الشحن هذا مسبقاً!"
        
        amount_usd = float(row[0])
        now_str = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
        cursor.execute('UPDATE vouchers SET is_used = 1, used_by = ?, used_at = ? WHERE code = ?', (str(user_id), now_str, clean_code))
        conn.commit()
        conn.close()
...
# ----------------- خادم ويب خفيف للفحص الصحي -----------------
class HealthCheckHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        self.send_response(200)
        self.send_header('Content-type', 'text/plain; charset=utf-8')
        self.end_headers()
        self.wfile.write(b"Number SMS Store Ultra Bot Engine Live 24/7!")
...
# ----------------- تهيئة البوت بتعدد خيوط ضخم (32 Workers) -----------------
...
def build_main_keyboard(user_id):
    ...
    markup.add(
        ...
        types.InlineKeyboardButton("🎟️ • شحن كود / كرت هدية •", callback_data="btn_redeem_voucher")
    )
...
def build_admin_main_keyboard():
    ...
    markup.add(
        types.InlineKeyboardButton("🎁 توليد كرت شحن (Voucher)", callback_data="admin_gen_voucher"),
...
def handle_all_text_messages(message):
    try:
        ...
        # 1. شحن كود / كرت هدية
        if action == "enter_voucher_code":
            success, res = redeem_voucher(text, message.from_user.id)
            if success:
                bal_usd, bal_rub, _, _, _, _ = get_or_create_user(message.from_user.id)
                success_msg = (
                    f"🎉 **مبروك! تم شحن الكرت بنجاح!**\n\n"
                    f"💵 المبلغ المضاف: **+{res:.2f} $** ({res * RUB_PER_USD:.1f} ₽)\n"
                    f"💰 رصيدك الحالي: **{bal_usd:.2f} $**\n\n"
                    f"استمتع بشراء أرقامك وتفعيل تطبيقاتك الآن!"
                )
                bot.reply_to(message, success_msg, reply_markup=types.InlineKeyboardMarkup().add(types.InlineKeyboardButton("☎️ شراء رقم الآن", callback_data="btn_buy_number")))
            else:
                bot.reply_to(message, f"{res}\n\n👉 تأكد من الكود وحاول مجدداً.")

        # 2. توليد كرت شحن بواسطة الأدمن
        elif action == "admin_gen_voucher_amount" and user_id_str == str(ADMIN_ID):
            try:
                amt = float(text)
                code = create_voucher(amt, user_id_str)
                if code:
                    card_text = (
                        f"🎟️ **تم توليد كرت الشحن بنجاح!**\n"
                        f"━━━━━━━━━━━━━━━━━━\n"
                        f"🔑 • **الكود :** `{code}`\n"
                        f"💰 • **القيمة :** **{amt:.2f} $** ({amt * RUB_PER_USD:.1f} ₽)\n"
                        f"━━━━━━━━━━━━━━━━━━\n"
                        f"💡 يمكنك نسخ الكود وإرساله للزبون ليشحنه فوراً بضغطة زر!"
                    )
                    bot.reply_to(message, card_text)
                else:
                    bot.reply_to(message, "❌ فشل توليد الكرت.")
            except Exception:
                bot.reply_to(message, "❌ يرجى إدخال رقم صحيح (مثال: 5 أو 10).")

        # 3. إدخال رقم عملية الدفع الآلي (جيب / بينانس)
        elif action == "verify_auto_payment":
            method = state.get("method", "jeeb")
            method_title = "محفظة جيب" if method == "jeeb" else "منصة بينانس"
            
            # إرسال إشعار فوري للمدير + المستخدم
            notify_admin = (
            ...
            bot.reply_to(
                ...
                f"⏳ جاري مطابقة العملية وإيداع الرصيد في محفظتك تلقائياً خلال ثوانٍ معدودة.",
        ...
        # 4. البحث عن دولة
            ...
            try:
                ...
                bot.reply_to(message, f"✅ **تم شحن {amount:.2f} $ ({amount * RUB_PER_USD:.1f} ₽) بنجاح للمستخدم `{target_uid}`.**")
                try:
                    bot.send_message(target_uid, f"🎉 **تم إيداع رصيد جديد في محفظتك!**\n\n💵 المبلغ: **{amount:.2f} $** ({amount * RUB_PER_USD:.1f} ₽)\n✅ استمتع بتفعيل أرقامك الآن.")
                except Exception:
                    pass
            except Exception:
...
def router_callback(call):
    try:
        ...
        # ----------------- قسم الشحن والدفع الآلي -----------------
            ...
            markup.add(types.InlineKeyboardButton("🎟️ شحن كرت هدية / كود جاهز", callback_data="btn_redeem_voucher"))
            ...
            text = (
                ...
                f"2. اضغط على زر **(⚡ تأكيد وتفعيل الدفع التلقائي)** أدناه وأرسل رقم العملية (المرجع) وسيتم شحن رصيدك تلقائياً!"
            )
            bot.edit_message_text(chat_id=chat_id, message_id=call.message.message_id, text=text, reply_markup=markup)
            ...
            text = (
                ...
                f"2. اضغط على زر **(⚡ تأكيد وتفعيل الدفع التلقائي)** وأرسل رقم العملية (Order ID)."
            )
            bot.edit_message_text(chat_id=chat_id, message_id=call.message.message_id, text=text, reply_markup=markup)
        ...
        # ----------------- شحن واسترداد كروت الهدايا -----------------
        elif data == "btn_redeem_voucher":
            user_states[str(user_id)] = {"action": "enter_voucher_code"}
            bot.send_message(
                chat_id,
                "🎟️ **أرسل الآن كود الشحن / كرت الهدية (مثال: `GIFT-5USD-XXXXXX`):**"
            )

        # ----------------- لوحة الإدارة وتوليد الكروت -----------------
            ...
            bot.edit_message_text(
                ...
                text="👑 **لوحة تحكم إدارة المتجر والمستخدمين وتوليد الكروت:**",
                reply_markup=markup
            )

        elif data == "admin_gen_voucher":
            if str(user_id) != str(ADMIN_ID):
                return
            user_states[str(user_id)] = {"action": "admin_gen_voucher_amount"}
            bot.send_message(chat_id, "🎁 **أرسل قيمة كرت الشحن بالدولار المراد توليده (مثال: 1 أو 5 أو 10):**")
        ...
        # ----------------- باقي تدفق الشراء -----------------
            ...
            bot.send_message(chat_id, "🔍 **أرسل الآن اسم الدولة أو رمز مفتاحها (مثال: البرازيل، روسيا، مصر، 55، أو 20):**")
            ...
            markup.add(
                types.InlineKeyboardButton(f"{c_info['cost_rub']} ₽ | {c_info['cost_usd']:.2f} $", callback_data=f"exec_buy_{app_code}_{c_id}"),
            ...
            if bal_usd < c_info['cost_usd'] and bal_rub < c_info['cost_rub'] and str(user_id) != str(ADMIN_ID):
                msg_text = (
                    ...
                    f"• سعر الرقم: **{c_info['cost_rub']} ₽ ({c_info['cost_usd']:.2f} $)**\n"
                    f"• رصيدك الحالي: **{bal_usd:.2f} $**\n\n"
            ...
            if success:
                ...
                cost_str = f"{c_info['cost_rub']} ₽ ({c_info['cost_usd']:.2f} $)"
...
def start_bot():
    ...
    print("🚀 BOT WITH AUTO-PAY, VOUCHERS & PROFIT-PROTECTION IS LIVE 24/7...")
